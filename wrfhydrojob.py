...
if __name__ == "__main__":
    ...
    while model_runner.pbs_id is None:
        logger.info("Waiting for PBS job ID to be set...")
        time.sleep(10)
    while True:
        logger.info("Waiting for PBS job status to be set...")
        time.sleep(10)
        model_runner.check_pbs_job_status()
        if 'C' in model_runner.job_status:
            logger.info("PBS job completed.")
            break
        elif 'E' in model_runner.job_status:
            logger.error("PBS job encountered an error.")
            break
        elif 'Q' in model_runner.job_status:
            logger.info("PBS job is queued.")
        else:
            logger.info("PBS job is running.")